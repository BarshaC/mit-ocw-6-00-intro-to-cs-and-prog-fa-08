...
class Hand(object):
    ...
    def containsLetters(self, letters):
        ...
        for char in newDict:
            if newDict[char] > self.handDict[char]:
                return False
            elif char not in self.handDict:
                return False
        else:
            return True

    def isValid(self, word):
        new_d = getFrequencyDict(word)
        for k in new_d:
            if k not in self.handDict:
                return False
            elif new_d[k] > self.handDict[k]:
                return False
        return True 
...
class ComputerPlayer(Player):
    ...
    PickBestWord method.
    """
    def pickBestWord(self, word_list_object):
        """
        ...
        the wordlist
        """
        newWordList = word_list_object.getList()
        bestWord = ''
        bestWordScore = 0
        hand = self.hand
        for word in newWordList:
            if hand.isValid(word):
                word_score = getWordScore(word)
                if word_score >= bestWordScore:
                    bestWordScore = word_score
                    bestWord = word
        if bestWordScore >= 1:
            return bestWord
        else:
