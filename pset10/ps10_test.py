...
def testHand():
    ...
    """
    h = Hand(8, {'a':3, 'b':2, 'd':3, 's':1})
    h.update('sad')
    testResult(h.containsLetters('aabbdd') and not h.isEmpty())
    h.update('dad')
...
def comparePlayers():
    p1 = Player(1, Hand(6, {'c':1, 'a':1, 'b':1 ,'d':1, 'o':1, 'e':1}))
    p2 = Player(2, Hand(8, {'a':3, 'b':2, 'd':3, 's':1}))
